        ...
        <div class="card-body">
            <div class="row row-xs align-items-center my-4 mg-b-20 d-flex align-items-stretch">
                <div class="col-md-4 mb-2">
                    ...
                    <input value="{{ $operation->patient->name }}" id="name" type="text" name="name"
                ...
                <div class="col-md-4 mb-2">
                    ...
                    <input value="{{ $operation->name }}" id="name" type="text" name="name" class="form-control"
                        disabled>
                </div>
                <div class="col-md-4 mb-2">
                    <label for="name" class="form-label">
            ...
            <div class="row row-xs align-items-center my-4 mg-b-20 d-flex align-items-stretch">
                <div class="col-md-4">
                    ...
                    <textarea id="description" rows="4" name="description" class="form-control" disabled
                        type="text">{{ $operation->description }}</textarea>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    @if(auth('admin')->check())
                    @else
                    <form action="{{ route('add_patient_operation_medicines', 'test') }}" method="post"
                        autocomplete="off" class="needs-validation" novalidate>
                        @endif
                        @csrf
                        <input value="{{ $operation->patient->id }}" type="hidden" name="patient_id">
                        <input value="{{ $operation->id }}" type="hidden" name="operation_id">
                        <input value="{{ $operation->invoice_id }}" type="hidden" name="invoice_id">
                        <label for="medicines">{{trans('Dashboard/operations.medicines')}}</label>
                        <select multiple class="form-control" id="medicines" name="medicines[]" required>
                            @foreach($medicines as $medicine)
                            <option value="{{$medicine->id}}">{{$medicine->name}}</option>
                            @endforeach
                        </select>
