            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('Payment.update', 'test') }}" method="post" autocomplete="off"
                        ...
                        {{ method_field('patch') }}
                        {{ csrf_field() }}
                        <input class="form-control" value="{{$payment_accounts->id}}" id="id" name="id" type="hidden">
                        <div class="row row-xs align-items-center mg-b-20">
                            <div class="col-md-6">
                                <label class="form-label" for="patient_id">
                                    {{trans('Dashboard/receipt.name_patient')}}
                                </label>
                                <select id="id" name="patient_id" class="form-control select2" required>
                                    <option value="" disabled>
                                        {{trans('Dashboard/invoices.choose_from_the_list')}}
                                    </option>
                                    @foreach($Patients as $Patient)
                                    <option value="{{$Patient->id}}" {{$payment_accounts->patient_id == $Patient->id
                            ...
                            <div class="col-md-6">
                                ...
                                <input value="{{$payment_accounts->amount}}" id="amount" type="number" min="0"
