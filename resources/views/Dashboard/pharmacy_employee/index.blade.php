...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('pharmacy_employee.create') }}" class=" d-none d-sm-inline-block btn btn-sm btn-primary
            shadow-sm">
                ...
                <table class="table table-bordered text-center" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{$pharmacy_employee->description}}</td>
                            <td>{{ $pharmacy_employee->email }}</td>
                            <td>
                                @if ($pharmacy_employee->schedules->count() > 0)
                                @foreach ($pharmacy_employee->schedules->slice(0, 2) as $schedule)
                                {{ $schedule->day }}..<br>
                                @endforeach
                                @else
                            ...
                            <td>
                                <div
                                    class=" rounded border border-{{$pharmacy_employee->status == 1 ? 'success' : 'danger'}}">
                        ...
                        @include('Dashboard.pharmacy_employee.update_password')
                        @include('Dashboard.pharmacy_employee.update_status')
                        @include('Dashboard.pharmacy_employee.deleteEmp')
                        @endforeach
                    </tbody>
                </table>
            </div>
