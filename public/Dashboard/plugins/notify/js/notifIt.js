...
(function (root, factory) {
    ...
    function notif(config) {
        ...
        var dismiss = function () {
            ...
            if (!defaults.fade) {
                ...
                if (
                    ...
                    defaults.animations[defaults.available_animations[0]] &&
                    defaults.animations[defaults.available_animations[0]][
                        defaults.position
                    ] &&
                    defaults.animations[defaults.available_animations[0]][
        ...
        defaults.animations.slide = {
            ...
            right: {
                ...
                out: {
                    start: {
                        right: parseFloat(
                            defaults.width - defaults.width * 0.9
                        ),
                    },
                    end: {
                        right: parseInt(0 - defaults.width * 2),
        ...
        // Set entry animation
        if (
            defaults.animations[defaults.animation][defaults.position].css_start
        ) {
            $("#ui_notifIt").css(
                defaults.animations[defaults.animation][defaults.position]
                    .css_start
        ...
        $("#ui_notifIt").animate(
            defaults.animations[defaults.animation][defaults.position].in
        );
        // Events
        if (!defaults.clickable) {
    ...
    function notif_prompt(config) {
        ...
        function _setListeners() {
            $("html").one(
                "click",
                ".notifit_prompt_accept, .notifit_prompt_cancel",
                _callback
            );
        }
